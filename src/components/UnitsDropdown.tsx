          ...
          <ul className="w-56 h-auto gap-y-1 flex flex-col">
            {items.map((item, index) => (
              <li key={index} className={`relative flex-col items-center gap-2 px-4 py-2`}>
                ...
                <button className={`text-preset-7 text-(--neutral-0) w-full h-10 px-2 py-2.5 inline-flex items-center justify-between text-left cursor-pointer hover:bg-(--neutral-600) rounded-lg ${appCtx.data.unit === 'metric' ? 'bg-(--neutral-600)' : ''}`} onClick={() => {appCtx.handleChange?.('unit','metric'); getWeatherData(appCtx, 
                  appCtx.data.location.latitude, 
                  appCtx.data.location.longitude)}} disabled={appCtx.data.unit === 'metric'} type="button">
                  {item.metricUnit}
                ...
                <button className={`text-preset-7 text-(--neutral-0) w-full h-10 px-2 py-2.5 inline-flex items-center justify-between text-left cursor-pointer hover:bg-(--neutral-600) rounded-lg ${appCtx.data.unit === 'imperial' ? 'bg-(--neutral-600)' : ''}`} onClick={() => { appCtx.handleChange?.('unit','imperial'); getWeatherData(appCtx, 
                  appCtx.data.location.latitude, 
                  appCtx.data.location.longitude)}} disabled={appCtx.data.unit === 'imperial'} type="button">
                  {item.imperialUnit}
